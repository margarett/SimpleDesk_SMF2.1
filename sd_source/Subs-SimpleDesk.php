...
function shd_init()
{
	...
	//From 2.1 onwards there is no more "Core Features" so this is always true :)
...
function shd_main_menu(&$menu_buttons)
{
	...
	if (!empty($modSettings['helpdesk_active']))
	{
		...
		// 1. Add the main menu if we can. Function_exists shouldn't be necessary but a strange error at installation occurs. Odd...
		if (function_exists('shd_allowed_to') && shd_allowed_to(array('access_helpdesk', 'admin_helpdesk'), 0) && empty($modSettings['shd_hidemenuitem']))
		{
			// Load our CSS for the menu.
			loadCSSFile('helpdesk_menu.css');
